...
Perfil Socioeconômico e Padrões de Sobrevivência dos passageiros do RMS Titanic}
...
Seu objetivo não é inferir parâmetros populacionais, mas sim organizar e
descrever os dados de forma clara e objetiva.
...
classificadas em qualitativas e quantitativas; essa caracterização inicial é
apresentada na Seção~\ref{subsec:caracterizacao_inicial}.
...
\textit{Tarifa 2024}. A composição detalhada desse índice é apresentada na
Seção~\ref{subsec:caracterizacao_inicial}.

\subsection{Faixas Etárias}

A variável idade contínua foi discretizada em quatro faixas etárias
definidas pelo autor com base em critérios demográficos intuitivos. As
faixas definidas foram: \textbf{Criança} (0--12 anos), \textbf{Jovem}
(12--17 anos), \textbf{Adulto} (17--59 anos) e \textbf{Idoso}
(59--100 anos). A distribuição observada em cada faixa é apresentada na
Seção~\ref{subsec:caracterizacao_inicial}.

\subsection{Procedimentos Analíticos}

A análise foi organizada em cinco etapas complementares. Primeiro, as
variáveis foram classificadas em qualitativas e quantitativas. Em seguida,
foram calculadas medidas de tendência central e dispersão para idade e
tarifa, incluindo quartis e percentis selecionados. Na terceira etapa, foram
construídos histogramas e curvas de densidade para descrever a forma das
distribuições. Na quarta etapa, variáveis qualitativas e perfis de
sobrevivência foram analisados por meio de gráficos de barras, pizza e barras
empilhadas proporcionais. Por fim, a relação entre idade e tarifa corrigida
foi examinada com gráfico de dispersão, matriz de correlação e coeficiente de
Pearson.
...
Antes das análises centrais, é importante registrar três saídas da etapa de
preparação dos dados: a identificação das variáveis, a composição do fator de
correção monetária e a distribuição da amostra por faixa etária.
...
A Tabela~\ref{tab:variaveis} mostra que o dataset combina variáveis
qualitativas, como gênero, classe e sobrevivência, com variáveis
quantitativas, como idade e tarifa. Essa separação orienta a escolha dos
métodos descritivos aplicados ao longo do trabalho.
...
A Tabela~\ref{tab:inflacao} resume o fator acumulado de 107,14 utilizado para
converter as tarifas originais de 1912 em valores equivalentes de 2024. Essa
transformação não altera a ordem relativa dos passageiros, mas torna a
interpretação econômica da tarifa mais concreta e atual.
...
Como mostra a Tabela~\ref{tab:faixas}, a amostra é fortemente concentrada em
adultos ($n=711$), com participação bem menor de crianças, jovens e idosos.
Essa composição ajuda a contextualizar os padrões observados nas análises de
...
\textit{p-value} associado. O valor obtido foi $r = 0{,}115$ e $p = 0{,}0007$.  
O \textit{p-value} indica que há uma probabilidade muito pequena (0,07\%) de
essa correlação ter ocorrido apenas por acaso, caso não existisse nenhuma
relação real entre as variáveis. Ainda assim, o valor de $r$ é muito baixo,
caracterizando uma correlação positiva \emph{muito fraca}.  
Na prática, isso significa que não há uma relação linear relevante entre as
variáveis analisadas. Os resultados da correlação são detalhados na

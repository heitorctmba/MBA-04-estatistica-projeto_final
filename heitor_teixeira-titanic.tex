...
informações dos passageiros a bordo no momento do naufrágio. Após a remoção
de registros com tarifa igual a zero — correspondentes a tripulantes e
passageiros sem custo de passagem registrado — a amostra final utilizada nas
análises compreende \textbf{872 passageiros}. As variáveis foram renomeadas
para o português e mapeadas conforme descrito na Tabela~\ref{tab:variaveis}.

\begin{figure*}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{table_3_2.png}
...
padronizadas segundo critérios demográficos usuais, conforme a
Tabela~\ref{tab:faixas}:
...
\begin{figure*}[ht]
  \centering
  \includegraphics[width=0.9\textwidth]{table_3_1.png}
...
grupo pagou valores superiores a £20.000 — reflexo da oferta de acomodações
...
reflete tanto o acesso preferencial aos botes salva-vidas quanto a
localização física dos camarotes — mais próximos ao convés no caso da
primeira classe.
...
homens apresentaram apenas 19\% — diferença de 54 pontos percentuais. Esse
padrão reflete diretamente o protocolo de evacuação \textit{``mulheres e
crianças primeiro''} adotado pela tripulação do Titanic.
...
maiores taxas de sobrevivência, consistentemente com o protocolo de
evacuação. Adultos, apesar de constituírem a maioria da amostra, apresentaram
as menores taxas relativas.
...
O coeficiente de Pearson obtido foi $r = 0{,}115$ ($p = 0{,}0007$). Embora
...
a taxa de sobrevivência variou de 24\% (terceira classe) a 63\% (primeira
classe), e de 19\% (homens) a 73\% (mulheres). Esses dados confirmam que a
sobrevivência no Titanic não foi aleatória, mas sistematicamente determinada
por fatores socioeconômicos e demográficos — classe, gênero e faixa etária.
